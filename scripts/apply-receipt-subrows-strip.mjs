...
if (!source.includes('const subReceiptCount = r.f.subReceipts?.length || 0;')) {
  replaceOnce(
    ...
    'счётчик доступных бланков',
  );
}

const titleWithToggle = `                                    <span className="rec-import-title"><ReceiptParticipantSummary draft={p} noun={r.f.type === 'Гостиница' ? 'гостей' : 'пассажиров'} />
...
const plainTitle = `                                    <span className="rec-import-title"><ReceiptParticipantSummary draft={p} noun={r.f.type === 'Гостиница' ? 'гостей' : 'пассажиров'} /></span>`;
if (source.includes(titleWithToggle)) {
  source = source.replace(titleWithToggle, plainTitle);
  sourceChanged = true;
}
...
if (!source.includes('className="receipt-subrows-inline-count"')) {
  if (!source.includes(typeSelect)) throw new Error('Не найден тип услуги для блока бланков.');
  source = source.replace(typeSelect, inlineBlanks);
  sourceChanged = true;
}
...
if (source.includes("<tr className={'receipt-subrows-strip-row'")) {
  throw new Error('Блок бланков всё ещё находится снаружи основной строки документа.');
}
...
console.log(sourceChanged || cssChanged
  ? 'Количество бланков и кнопка раскрытия встроены внутрь блока документа.'
  : 'Блок бланков уже находится внутри документа.');
