...
function ServiceCardAdminDrawer({ onClose }) {
  ...
  return (
    <StackPanel title={t("Настройки карточек услуг · администратор")} width="min(1080px,97vw)" onClose={onClose}
      footer={<>
        <div style={{ fontSize: 12, color: 'var(--muted)' }}>{t("Изменения применяются ко всей системе карточек услуг")}</div>
        <div style={{ flex: 1 }} />
        <Button variant="secondary" onClick={onClose} disabled={busy}>{t("Отмена")}</Button>
        <Button icon="check" onClick={save} disabled={busy || !loaded}>{busy ? 'Сохранение…' : 'Сохранить'}</Button>
      </>}>
      ...
      {tab === 'kinds' && (
        <div className="grid-2" style={{ gap: 20, alignItems: 'start' }}>
          <div className="card card-pad">
            <h3 className="card-title" style={{ fontSize: 14, margin: '0 0 4px' }}>{t("Виды услуг с карточками")}</h3>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>{t("Включайте и отключайте виды карточек услуг (§30).")}</div>
          ...
          <div className="card card-pad">
            <h3 className="card-title" style={{ fontSize: 14, margin: '0 0 4px' }}>{t("Каналы отправки")}</h3>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>{t("Доступные каналы для отправки карточек клиенту (§20, §30).")}</div>
      ...
      {tab === 'email' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>{t("Шаблоны письма по сценарию. Подстановки:")}{'{label}'}{t("— ярлык,")}{'{title}'}{t("— услуга.")}</div>
          {Object.keys(emails).map((sys) => (
            <div key={sys} className="card card-pad">
              ...
              <label className="lbl" style={{ display: 'block', marginBottom: 6 }}>{t("Тема")}</label>
              <Input value={emails[sys].subject} onChange={(e) => setEmails((s) => ({ ...s, [sys]: { ...s[sys], subject: e.target.value } }))} />
              <label className="lbl" style={{ display: 'block', margin: '10px 0 6px' }}>{t("Текст письма")}</label>
      ...
      {tab === 'fields' && (
        <div className="card card-pad" style={{ maxWidth: 560 }}>
          <h3 className="card-title" style={{ fontSize: 14, margin: '0 0 4px' }}>{t("Видимость полей для клиента")}</h3>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>{t("Какие финансовые поля клиент видит в карточке. Неотмеченные остаются внутренними (§7).")}</div>
    ...
    <div className="grid-2" style={{ gap: 20, alignItems: 'start', gridTemplateColumns: '260px 1fr' }}>
      ...
      <div className="card card-pad">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--muted)' }}>{t("финансовая логика:")}{sc.fin}</span>
          <div style={{ flex: 1 }} />
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>{t("Включён")}<Toggle on={sc.enabled !== false} onChange={(on) => upd({ enabled: on })} /></label>
        </div>

        <div className="grid-2" style={{ gap: 12 }}>
          <div><label className="lbl" style={{ display: 'block', marginBottom: 6 }}>{t("Клиентское название сценария")}</label><Input value={sc.name} onChange={(e) => upd({ name: e.target.value })} /></div>
          <div><label className="lbl" style={{ display: 'block', marginBottom: 6 }}>{t("Основной ярлык")}</label><Input value={sc.badge} onChange={(e) => upd({ badge: e.target.value })} /></div>
        </div>

        <label className="lbl" style={{ display: 'block', margin: '12px 0 6px' }}>{t("Варианты ярлыка (по строке)")}</label>
        ...
        <label className="lbl" style={{ display: 'block', margin: '14px 0 6px' }}>{t("Действия клиента")}</label>
        ...
        <label className="lbl" style={{ display: 'block', margin: '14px 0 6px' }}>{t("Блоки карточки и порядок")}</label>
