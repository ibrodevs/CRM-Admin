...
export function RouteRenderer({ route, role, auth, orders, suppliers, workspace, navigate, createOrder, openOrder, createOrderFromPicker, openChat, intent, setIntent, addOrder, setCtxOrder, openServiceSearch, svcSearch, setSvcSearch, addSupplier, openChatThread, focusedChat, createReceiptOrder, toast }) {
  const page = (
      <>
      {route === 'dashboard' && <DashboardPage role={role} user={auth.user} orders={orders} orderServices={workspace.orderServices} clients={workspace.clients} companies={workspace.companies} proposals={workspace.proposals} returns={workspace.returns} chats={workspace.chats} dashboard={workspace.dashboard} calendar={workspace.calendar} finance={workspace.finance} resources={workspace.resources} incidents={workspace.integrationIncidents} slaQueue={workspace.slaQueue} currentShift={workspace.currentShift} motivationAccruals={workspace.motivationAccruals} users={workspace.users} suppliers={workspace.suppliers} onNavigate={navigate} onAddOrder={createOrder} onOpenOrder={openOrder} onCreateOrder={createOrderFromPicker} onOpenChat={openChat} onReload={() => workspace.reload()} />}
      ...
      {route === 'orders' && <OrdersPage intent={intent} onConsume={() => setIntent(null)} orders={orders} clients={workspace.clients} companies={workspace.companies} addOrder={addOrder} onOrderUpdated={(order) => workspace.update('orders', (rows) => [order, ...rows.filter((row) => row.id !== order.id)])} onDetailChange={setCtxOrder} onOpenChat={openChat} onNavigate={navigate} currentUser={auth.user} />}
