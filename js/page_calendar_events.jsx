...
function calRepeatSummary(every, unit, until = '') {
  const count = Math.max(1, Number(every) || 1);
  const forms = {
    'Час': ['час', 'часа', 'часов'],
    'День': ['день', 'дня', 'дней'],
  ...
  const wordForms = forms[unit] || forms['День'];
  const mod10 = count % 10;
  const mod100 = count % 100;
  const word = mod10 === 1 && mod100 !== 11 ? wordForms[0] : mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14) ? wordForms[1] : wordForms[2];
  const prefix = count === 1 ? (unit === 'Неделя' ? 'Каждую' : 'Каждый') : `Каждые ${count}`;
  return `${prefix} ${word}${until ? ` · до ${until}` : ' · без даты окончания'}`;
}

function calRepeatPreview(dateStr, time, every, unit, until = '') {
  const [dd, mm, yy] = String(dateStr || '').split('.').map(Number);
  const [hh, mi] = String(time || '12:00').split(':').map(Number);
  const start = new Date(yy || new Date().getFullYear(), (mm || 1) - 1, dd || 1, hh || 12, mi || 0);
  const [endDay, endMonth, endYear] = String(until || '').split('.').map(Number);
  const end = until ? new Date(endYear || yy, (endMonth || 1) - 1, endDay || 1, 23, 59, 59) : null;
  const count = Math.max(1, Number(every) || 1);
  const rows = [];
  for (let index = 1; index <= 4; index += 1) {
    const next = new Date(start);
    if (unit === 'Час') next.setHours(next.getHours() + count * index);
    else if (unit === 'Неделя') next.setDate(next.getDate() + count * index * 7);
    else if (unit === 'Месяц') next.setMonth(next.getMonth() + count * index);
    else next.setDate(next.getDate() + count * index);
    if (end && next > end) break;
    rows.push(next.toLocaleString('ru-RU', { day: '2-digit', month: 'long', hour: '2-digit', minute: '2-digit' }).replace(',', ' ·'));
  }
  return rows;
}
...
function CalCustomRepeatDrawer({ value, dateStr, time, onClose, onApply }) {
  ...
  const preview = calRepeatPreview(dateStr, time, every, unit, until);
  const valid = Number(every) >= 1;
  return (
    <Drawer open onClose={onClose} title="Свой интервал повторения"
      sub="Настройте, как часто и до какой даты повторять событие" width="min(520px,96vw)"
      footer={<>
        <Button variant="secondary" style={{ flex: 1 }} onClick={onClose}>Отмена</Button>
        <Button icon="check" style={{ flex: 1 }} disabled={!valid}
          onClick={() => onApply({ every: String(Math.max(1, Number(every) || 1)), unit, until })}>Применить интервал</Button>
      </>}>
      <div className="cal-repeat-intro">
        <Icon name="clock" />
        <div><b>Частота повторения</b><span>Например: каждые 2 дня или каждый месяц</span></div>
      </div>
      <div className="cal-repeat-frequency">
        <Field label="Повторять каждые" required>
          <Input type="number" min="1" value={every} onChange={(event) => setEvery(event.target.value.replace(/[^\d]/g, ''))} placeholder="2" />
        </Field>
        <Field label="Период" required>
          <Select value={unit} onChange={(event) => setUnit(event.target.value)} options={CAL_REPEAT_UNITS} />
        </Field>
      </div>
      <UFDateField label="Закончить повторение" hint="необязательно" value={until || null} onChange={setUntil} placeholder="Без даты окончания" />
      <div className="cal-repeat-result">
        <span>Будет повторяться</span>
        <b>{calRepeatSummary(every, unit, until)}</b>
      </div>
      <div className="cal-repeat-preview">
        <div className="cal-repeat-preview-title">Ближайшие повторения</div>
        {preview.length ? preview.map((date, index) => (
          <div key={date} className="cal-repeat-preview-row">
            <span>{index + 1}</span><Icon name="calendar" /><b>{date}</b>
          </div>
        )) : <div className="cal-repeat-preview-empty">До выбранной даты новых повторений не будет</div>}
      </div>
    </Drawer>
  );
}
...
function calendarEventToUi(item, orders = [], users = [], clients = [], suppliers = []) {
  const order = orders.find((entry) => entry.id === item.order);
  const assignee = users.find((entry) => entry.id === item.assignee);
  const person = clients.find((entry) => entry.id === item.person);
  const supplier = suppliers.find((entry) => entry.id === item.supplier || entry.no === item.supplier);
  const date = new Date(item.starts_at);
  return {
    ...item, type: CAL_KIND_FROM_BACKEND[item.kind] || item.kind, date,
    time: date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }),
    order: order?.no || item.order || null, resp: assignee?.full_name || assignee?.name || 'Не назначен',
    priority: CAL_PRIORITY_FROM_BACKEND[item.priority] || item.priority,
    pax: person?.name || '', supplier: supplier?.name || '', done: item.status === 'done',
  };
}
function hydrateCalendarEvents(items = [], orders = [], users = [], clients = [], suppliers = []) {
  CAL_EVENTS.splice(0, CAL_EVENTS.length, ...items.map((item) => calendarEventToUi(item, orders, users, clients, suppliers)));
}
function calEventsOn(day) { return CAL_EVENTS.filter((e) => trSameDay(e.date, day)); }
function calAddEvent(evt) {
  const who = (window.CURRENT_USER && CURRENT_USER.name) || 'Пользователь';
...
function calOrderInfo(no) {
  const source = window.CALENDAR_ORDERS || [];
  const o = source.find((x) => x.no === no) || null;
  if (!o) return null;
  return { no: o.no, client: o.client, company: o.requestType, services: o.services, operator: o.operator };
}
    ...
    <div>
      ...
      {info && (
        <div style={{ marginTop: 8, padding: '10px 12px', borderRadius: 10, background: 'var(--surface-2)', fontSize: 12.5, display: 'grid', gap: 3 }}>
          ...
          <div style={{ color: 'var(--muted)' }}>Услуг: {info.services} · ответственный: {info.operator}</div>
        </div>
      )}
    </div>
  );
...
function CalPaxPickDrawer({ order, onPick, onClose, clients = [], users = [] }) {
  ...
  const passengers = clients.map((c) => ({
  ...
  const employees = users.map((user) => ({ id: user.id, name: user.full_name || user.name || user.email }))
...
function CalEventCreator({ type, day, presetOrder, orders = [], clients = [], users = [], suppliers = [], onClose, onCreated, onPersist }) {
  ...
  const submit = async () => {
    ...
    if (type === 'order') {
      try {
        const created = calUpsertEvent(await onPersist({ ...base, type, form: f, date, endStr: f.endStr }));
  ...
  const operatorNames = calendarOperatorRows(users).map((item) => item.name);
  ...
  return (
    <Drawer open title={t.l} sub="Заполните поля и создайте событие" width="min(640px,96vw)" onClose={onClose}
      ...
      {type === 'order' && (<>
        ...
        <Field label="Существующий заказ"><CalOrderPicker value={f.order} orders={orders} onChange={(v) => set('order', v)} /></Field>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <Field label="Ответственный оператор"><Select value={f.resp} onChange={(e) => set('resp', e.target.value)} options={f.resp && !operatorNames.includes(f.resp) ? [f.resp, ...operatorNames] : operatorNames} /></Field>
        </div>
      </>)}
      ...
      {type !== 'order' && (<>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          {type === 'reminder' && <Field label="Поставщик"><Select value={f.supplier} onChange={(e) => set('supplier', e.target.value)} options={['', ...suppliers.map((item) => item.name)]} placeholder="(необязательно)" /></Field>}
    ...
    <Drawer open={!!evt} onClose={onClose} title={evt.title} sub={t.l + ' · ' + calFmtDay(evt.date) + (evt.time ? ' · ' + evt.time : '')}
      ...
      {info && (
        <div className="card card-pad" style={{ marginBottom: 14, background: 'var(--surface-2)' }}>
          ...
          <div style={{ fontSize: 12.5, color: 'var(--muted)' }}>Клиент: {info.client} · услуг: {info.services} · ответственный: {info.operator}</div>
        </div>
      )}
    </Drawer>
  );
