...
function receiptWithPricing(type, receipt, pricing) {
  ...
  if (type === 'ЖД') {
    ...
    const reservedSeatCost = pricing?.reservedSeatCost !== undefined && pricing?.reservedSeatCost !== null && pricing?.reservedSeatCost !== ''
      ? round(pricing.reservedSeatCost)
      : round(receipt?.reservedSeatCost);
    const ticketCost = pricing?.ticketCost !== undefined && pricing?.ticketCost !== null && pricing?.ticketCost !== ''
...
function ReceiptMathDrawer({ open, file, math, scopeOptions = [], feeInfo = null, onSave, onClose }) {
  ...
  const buildInitialMath = (baseMath, parsed) => {
    const raw = baseMath || {};
    const parsedTicket = Number(parsed?.ticketCost ?? parsed?.ticket_cost);
    const parsedReserved = Number(parsed?.reservedSeatCost ?? parsed?.reserved_seat_cost);
    const ticketCost = raw.ticketCost !== undefined ? raw.ticketCost : (Number.isFinite(parsedTicket) ? parsedTicket : (raw.tariff || 0));
    const reservedSeatCost = raw.reservedSeatCost !== undefined ? raw.reservedSeatCost : (Number.isFinite(parsedReserved) ? parsedReserved : 0);
    const tariff = isRail
      ? num(num(ticketCost) + num(reservedSeatCost))
      : (raw.tariff ?? 0);
    return {
      tariff,
      ticketCost,
  ...
  useEffect(() => { if (open && math) setM(buildInitialMath(math, file?.parsed)); }, [open, file && file.id, math]);
    ...
    <Drawer open={open} onClose={onClose} title={'Внутренняя математика · ' + serviceMeta.title}
      ...
      <div className="receipt-internal-math-fields">
        ...
        {isRail ? (
          <>
            <label className="receipt-internal-math-field">
              <span>Стоимость билета, {sym}<small> · проезд по ЖД</small></span>
              <input
                className="input"
                type="number"
                min="0"
            ...
            <label className="receipt-internal-math-field">
              <span>Стоимость плацкарты, {sym}<small> · спальное / посадочное место</small></span>
              <input
                className="input"
                type="number"
                min="0"
