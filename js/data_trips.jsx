...
// Единый оперативный центр мониторинга: поездка = единая сущность из нескольких
// услуг. Модель данных + интеллектуальные проверки (события, конфликты, форс-мажоры,
// уровни критичности, лента «Центр контроля»). Всё считается от TRIP_NOW.

/* Опорное «сейчас» демо — привязано к датам демо-услуг (конец июня 2026),
   чтобы были поездки сегодня, ближайшие вылеты, конфликты и форс-мажоры. */
const TRIP_NOW = new Date(2026, 5, 24, 9, 30); // 24.06.2026 09:30

/* Уровни критичности события (ТЗ2: критический / высокий / средний / информационный). */
const TRIP_CRIT = {
...
/* Каталог форс-мажоров (ТЗ2: 13 видов) — тип → ярлык, критичность по умолчанию, иконка. */
...
/* Тип услуги внутри поездки: kind + временные метки + статус выписки/оплаты. */
// service: { kind, title, sub, supplier, status, start, end, from, to, pax, paid, ticketed,
//            checkinUntil, timeLimit, regOpens }

/* ---------- модель поездок (единая сущность) ---------- */
const TRIPS = [
  // T1 — мультимаршрут «Москва → Стамбул → Дубай», вылет сегодня (ожидается вылет)
  {
  ...
  // T2 — форс-мажор: задержка рейса сегодня вечером + позднее заселение + перенос трансфера
  {
  ...
  // T3 — групповая поездка (одна карточка на группу)
  {
  ...
  // T4 — согласование + истекает тайм-лимит (ожидается согласование / выписка)
  {
  ...
  // T5 — конфликт времени: трансфер раньше прибытия поезда
  {
  ...
  // T6 — неоплаченная услуга (ожидается оплата)
  {
  ...
  // T7 — спокойная будущая поездка (заезд/выезд, без проблем)
  {
  ...
  // T8 — стыковочный рейс с короткой пересадкой + заселение сильно позже прилёта
  {
    ...
    services: [
      ...
      { kind: 'Авиа', title: 'IST → LHR', sub: 'Turkish Airlines · TK 1979 · 1 пасс.', supplier: 'Amadeus GDS', status: 'Забронировано',
        ...
        start: D(2026, 5, 27, 12, 0), end: D(2026, 5, 27, 15, 30) }, // пересадка 30 мин
      { kind: 'Гостиница', title: 'Park Plaza London · 4★', sub: 'Standard · 3 ночи · BB', supplier: 'Booking B2B', status: 'Забронировано',
        ...
        start: D(2026, 5, 27, 21, 0), end: D(2026, 5, 30, 12, 0), checkinUntil: D(2026, 5, 28, 2, 0) }, // заселение через 5.5 ч после прилёта
    ],
  ...
  // T9 — ошибки оформления: два рейса одновременно, две гостиницы, конфликт операторов
  {
    ...
    services: [
      ...
      { kind: 'Авиа', title: 'FRU → DXB', sub: 'Emirates · EK 878 · 1 пасс.', supplier: 'Emirates (API)', status: 'Забронировано',
        ...
        start: D(2026, 5, 28, 10, 0), end: D(2026, 5, 28, 13, 0), timeLimit: D(2026, 5, 26, 18, 0) }, // пересекается с FRU→IST
      ...
      { kind: 'Гостиница', title: 'Swissotel Istanbul · 5★', sub: 'Superior · 3 ночи · BB', supplier: 'Booking B2B', status: 'Забронировано',
        from: 'Стамбул', to: '', pax: 1, paid: true, ticketed: true,
        start: D(2026, 5, 29, 14, 0), end: D(2026, 6, 1, 12, 0), checkinUntil: D(2026, 5, 29, 23, 0) }, // пересекается с Hilton
    ],
...
/* ---------- события поездки (индикаторы) ---------- */
// Возвращает [{ label, icon, crit, at, service }] — что и когда ожидается.
function tripEvents(trip, now = TRIP_NOW) {
  ...
  const soon = (dt, hoursCrit, hoursHigh, hoursMed) => {
    ...
    if (h < -2) return null;                 // уже прошло давно — не показываем
...
/* ---------- интеллектуальные проверки конфликтов ---------- */
// Возвращает [{ crit, text }] — конфликты стыковок/размещения внутри поездки.
function tripConflicts(trip) {
  const out = [];
  const svc = trip.services;
  const flights = svc.filter((s) => s.kind === 'Авиа');
  const arrivals = svc.filter((s) => s.kind === 'Авиа' || s.kind === 'ЖД'); // прибытие = .end, отправление = .start
  const transfers = svc.filter((s) => s.kind === 'Трансфер');
  const hotels = svc.filter((s) => s.kind === 'Гостиница');
  const overlap = (a, b) => a.start < b.end && b.start < a.end;

  // 1. Трансфер назначен раньше прибытия / недостаточно времени на встречу
  ...
  // Ближайшее прибытие перед заселением в тот же день (чтобы проверки не дублировались по каждому сегменту).
  const arrivalBefore = (h) => arrivals
    .filter((a) => trSameDay(a.end, trStartOfDay(h.start)) && a.end <= h.start)
    .sort((x, y) => y.end - x.end)[0];

  // 2. Позднее заселение (прибытие позже времени приёма гостиницы)
  hotels.forEach((h) => {
  ...
  // 3. Гостиница начинается позже прилёта (большой разрыв — клиенту негде разместиться)
  hotels.forEach((h) => {
    const a = arrivalBefore(h);
  ...
  // 4. Выселение происходит позже отправления
  ...
  // 5. Недостаточно времени на пересадку (между стыковочными рейсами)
  ...
  // 6. У одного пассажира два рейса одновременно
  ...
  // 7. Одновременно забронированы две гостиницы
  ...
  // 8. Задержанный рейс → трансфер нужно перенести
  ...
  // 9. Два оператора выполняют несовместимые действия
  if (trip.opsConflict) out.push({ crit: 'high', text: trip.opsConflict });

  // Группа: пассажиры без мест / невыписанные брони / размещение
  if (trip.group) {
...
/* Форс-мажоры поездки (отдельно от обычных событий, ТЗ2). Нормализуем по каталогу
   из 13 видов: тип → ярлык / критичность по умолчанию / иконка. */
...
/* Итоговая критичность поездки — максимум по событиям/конфликтам/форс-мажорам. */
...
/* Есть ли неоплаченные услуги. */
function tripUnpaid(trip) { return trip.services.some((s) => !s.paid); }

/* Лента «Центр контроля» — события ближайших часов по всем поездкам, отсортированы. */
...
/* Кросс-поездочные конфликты (совпадение командировок одного клиента и т.п.). */
...
/* Множества значений для фильтров. */
